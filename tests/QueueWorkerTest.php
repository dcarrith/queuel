...
class QueueWorkerTest extends PHPUnit_Framework_TestCase {
	...
	{
		$worker = $this->getMock('Dcarrith\Queuel\Worker', array('process'), array($manager = m::mock('Illuminate\Queue\QueueManager')));
		...
		$job = m::mock('Dcarrith\Queuel\Jobs\QueuelJob');
	...
	{
		$worker = $this->getMock('Dcarrith\Queuel\Worker', array('process', 'sleep'), array($manager = m::mock('Illuminate\Queue\QueueManager')));
		$manager->shouldReceive('connection')->once()->with('connection')->andReturn($connection = m::mock('StdClass'));
	...
	{
		$worker = new Dcarrith\Queuel\Worker(m::mock('Illuminate\Queue\QueueManager'), $failer = m::mock('Illuminate\Queue\Failed\FailedJobProviderInterface'));
		$job = m::mock('Dcarrith\Queuel\Jobs\QueuelJob');
	...
	{
		$worker = new Dcarrith\Queuel\Worker(m::mock('Illuminate\Queue\QueueManager'));
		$job = m::mock('Dcarrith\Queuel\Jobs\QueuelJob');
